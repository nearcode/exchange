<template>
    <transition name="slide">
        <!-- 提现记录 -->
        <scroll class="withdrawalRecord">
            <div class="record-list">
                <div class="list jus-b" v-for="item in withdrawalList" @click="toBillingDetail(item)">
                    <!-- 币种 -->
                    <div class="img-btc">
...
export default {
    ...
    data(){
        return{
            withdrawalList:[],//提现记录列表
            coin:this.$route.query.coin,//币种
        }
    },
    mounted(){
        console.log(this.coin)
        this.$api.user.getWithdrawRecord(this.coin).then(res =>{
            this.withdrawalList = res.historyList;
        })
